...
#QUEUES=$(for queue in `aws batch describe-job-queues --output text --query jobQueues[].[jobQueueName]` ; do echo $queue ; done)
QUEUES=$($AWSCMD batch describe-job-queues --query jobQueues[].[jobQueueName])
...
for queue in $QUEUES ; do 
	for status in SUCCEEDED FAILED ;do 
		$AWSCMD batch list-jobs --job-queue $queue --job-status $status --query jobSummaryList[*].[jobName,jobId] | while IFS=$'\t' read -r jobName jobId ; do
